...
import com.yjh.system.action.dataimport.util.StringUtil;
...
/**
 * 
* @ClassName: Excel2003RowProcessor 
* @Description: Excel-2003行级处理器，ExcelRowProcessor的实现类
* @author zhangwanxi
* @date 2013-7-22 上午11:03:56 
*
 */
public abstract class Excel2003RowProcessor implements ExcelRowProcessor{
	...
	/**
	 * 
	* @ClassName: MyHSSFListener 
	* @Description: 辅助实现类，HSSF监听器
	* @author zhangwanxi
	* @date 2013-7-22 上午11:04:21 
	*
	 */
		...
		public void processRecord(Record record) {
			...
			case NumberRecord.sid:
                ...
                if(!HSSFDateUtil.isInternalDateFormat(numrec.getXFIndex())){  
                	thisStr=(new SimpleDateFormat("yyyy-MM-dd")).format(HSSFDateUtil.getJavaDate(numrec.getValue()));  
                   // System.out.println("日付："+numrec.getValue()+", 行:"+numrec.getRow()+ ", 列："+numrec.getColumn()+"。（"+numrec.getXFIndex()+"）");   
                	
                    //addDataAndrChangeRow(numrec.getRow(),numrec.getColumn(),timeStr);  
                }else{  
                	thisStr = String.valueOf(numrec.getValue());
                   // System.out.println("数字："+numrec.getValue()+", 行:"+numrec.getRow()+ ", 列："+numrec.getColumn()+"。（"+numrec.getXFIndex()+"）");    
                   // addDataAndrChangeRow(numrec.getRow(),numrec.getColumn(),getTime(numrec.getValue()));  
                }  
				break;
			case RKRecord.sid:
