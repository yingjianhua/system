Ext.define('mvc.view.cst.CstSalInvoiceRed.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		this.store=Ext.create('mvc.store.cst.CstSalInvoiceRed');
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/cst_CstSalInvoiceRed_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
