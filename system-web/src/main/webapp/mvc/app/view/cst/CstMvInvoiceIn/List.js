Ext.define('mvc.view.cst.CstMvInvoiceIn.List',{
extend : 'Ext.panel.Panel',
oldId : 'CstMvInvoiceIn_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.cst.CstMvInvoiceIn.ListMain',{
							title : '调入发票汇总单',
	        ...
	        selectionchange: function(model, records) {
	            if (records.length === 1){
	                this.mdMain.getForm().loadRecord(records[0]);
									...
									this.mdOriginTable.store.filter([{
												'id' : 'filter',
												'property' : 'mainForm',
												'value' : records[0].get('bean.pkey')+'06205'//调入发票汇总单
											}]);
				...
				xtype : Ext.create('mvc.view.cst.CstMvInvoiceIn.ListLineCstMvInvoiceInLine',{
							title : '调入发票汇总单明细',
...
onIns : function(){
		var win = Ext.create('mvc.view.frm.FrmPending.WinPending',{
			title : this.title+'>新增',
			descUrl : 'cst_CstMvInvoiceIn_ins',
			descType : 6205, //目标单据类型
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/cst_CstMvInvoiceIn_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '调入发票汇总单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/cst_CstMvInvoiceIn_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.cst.CstMvInvoiceIn',result);
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '调入发票汇总单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/cst_CstMvInvoiceIn_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.cst.CstMvInvoiceIn',result);
...
onDoNote : function(){
	...
	if (selection){
		var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
			...
			insFlag : false,
			tableCode : 'irille.pss.cst.CstMvInvoiceIn'
		});
		win.show();
...
onDoTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '调入发票汇总单['+selection.get('bean.code') + '] - 记账确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/cst_CstMvInvoiceIn_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.cst.CstMvInvoiceIn',result);
...
onUnTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '调入发票汇总单['+selection.get('bean.code') + '] - 记账取消确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/cst_CstMvInvoiceIn_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.cst.CstMvInvoiceIn',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.cst.CstMvInvoiceIn.WinSearch',{
