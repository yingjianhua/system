Ext.define('mvc.view.cst.CstOutRed.List',{
extend : 'Ext.panel.Panel',
oldId : 'CstOutRed_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.cst.CstOutRed.ListMain',{
							title : '红字出库记账单',
	        ...
	        selectionchange: function(model, records) {
	            if (records.length === 1){
	                this.mdMain.getForm().loadRecord(records[0]);
									...
									this.mdOriginTable.store.filter([{
												'id' : 'filter',
												'property' : 'mainForm',
												'value' : records[0].get('bean.pkey')+'06208'//红字出库核算单
											}]);
				...
				xtype : Ext.create('mvc.view.cst.CstOutRed.ListLineCstOutRedLine',{
							title : '红字出库单明细表',
...
onIns : function(){
		var win = Ext.create('mvc.view.frm.FrmPending.WinPending',{
			title : this.title+'>新增',
			descUrl : 'cst_CstOutRed_ins',
			descType : 6208, //目标单据类型
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/cst_CstOutRed_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '红字出库记账单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/cst_CstOutRed_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.cst.CstOutRed',result);
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '红字出库记账单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/cst_CstOutRed_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.cst.CstOutRed',result);
...
onDoNote : function(){
	...
	if (selection){
		var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
			...
			insFlag : false,
			tableCode : 'irille.pss.cst.CstOutRed'
		});
		win.show();
...
onDoTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '红字出库记账单['+selection.get('bean.code') + '] - 记账确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/cst_CstOutRed_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.cst.CstOutRed',result);
...
onUnTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '红字出库记账单['+selection.get('bean.code') + '] - 记账取消确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/cst_CstOutRed_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.cst.CstOutRed',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.cst.CstOutRed.WinSearch',{
