Ext.define('mvc.view.pur.PurProtGoodsApply.List',{
extend : 'Ext.panel.Panel',
oldId : 'PurProtGoodsApply_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{xtype : 'textfield',name : 'bean.code',fieldLabel : '单据号'}
							...
							,{xtype : 'textfield',name : 'bean.name',fieldLabel : '供应商名称'}
							]
					}]
			}]
	},{
		...
		items : [{
				xtype : Ext.create('mvc.view.pur.PurProtGoodsApply.ListMain',{
							title : '供应商货物协议申请单',
				...
				xtype : Ext.create('mvc.view.pur.PurProtGoodsApply.ListLinePurProtGoodsApplyLine',{
							title : '供应商货物协议申请单明细',
...
onIns : function(){
		var win = Ext.create('mvc.view.pur.PurProtGoodsApply.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pur_PurProtGoodsApply_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '供应商货物协议申请单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/pur_PurProtGoodsApply_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.pur.PurProtGoodsApply',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.pur.PurProtGoodsApply.WinSearch',{
