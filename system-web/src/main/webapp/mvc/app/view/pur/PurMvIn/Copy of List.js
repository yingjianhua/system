...
mainActs.push({
		...
		if (this.roles.indexOf('insDr') != -1)
mainActs.push({
		text : '调入直销',
		iconCls : 'ins-icon',
		itemId : this.oldId+'insDr',
		scope : this,
		handler : this.onInsDr
	});
				                ...
				                selectionchange: function(model, records) {
        							...
        							if (status==STATUS_INIT){
	    								if (this.roles.indexOf('upd') != -1)
											...
											if (this.roles.indexOf('cIn') != -1)
												this.down('#'+this.oldId+'cIn').setDisabled(true);
        							...
        							if (this.roles.indexOf('upd') != -1)
											...
											if (this.roles.indexOf('cIn') != -1)
												this.down('#'+this.oldId+'cIn').setDisabled(false);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pur_PurMvIn_delMulti',
			...
			Ext.MessageBox.confirm(msg_confirm_title, '调入单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/pur_PurMvIn_approve?pkey='+selection.get('bean.pkey'),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
			...
			Ext.MessageBox.confirm(msg_confirm_title, '调入单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/pur_PurMvIn_unapprove?pkey='+selection.get('bean.pkey'),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
...
onCIn : function(){
		...
		if (selection){
			Ext.Ajax.request({
				url : base_path+'/pur_PurMvIn_cIn?pkey='+selection.get('bean.pkey'),
				success : function (response, options) {
					var result = Ext.decode(response.responseText);
					if (result.success){
