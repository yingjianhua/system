Ext.define('mvc.view.pur.PurMvIn.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '调入类型',width : 100,dataIndex : 'bean.mvType',sortable : true,renderer : mvc.Tools.optRenderer('pur','PurMvIn','OMvType')}
	,{text : '货物去向',width : 100,dataIndex : 'bean.goodsTo',sortable : true,renderer : mvc.Tools.optRenderer('pur','PurMvIn','OGoodsTo')}
	...
	,{text : '调出单',width : 80,dataIndex : 'bean.outForm',sortable : true,renderer : mvc.Tools.beanRenderer()}
	,{text : '调出机构',width : 100,dataIndex : 'bean.orgOther',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysOrg.List'}
	,{text : '调出核算单元',width : 100,dataIndex : 'bean.cellOther',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysCell.List'}
	,{text : '调出仓库',width : 100,dataIndex : 'bean.warehouseOther',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'gs',mn : 'view.gs.GsWarehouse.List'}
	,{text : '单据来源',width : 100,dataIndex : 'bean.fromType',sortable : true,renderer : mvc.Tools.optRenderer('pur','PurMvIn','OFromType')}
	...
	,{text : '审核时间',width : 140,dataIndex : 'bean.apprTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s'),expandCol : true,hidden : true}
		...
		this.store=Ext.create('mvc.store.pur.PurMvIn');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.pur.PurMvIn', data);
...
onUpdWin : function(selection){
		if (selection){
			...
			mvc.model.pur.PurMvIn.load(selection.get('bean.pkey'), {
				scope : this,
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/pur_PurMvIn_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
