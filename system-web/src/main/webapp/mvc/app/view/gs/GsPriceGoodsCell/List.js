Ext.define('mvc.view.gs.GsPriceGoodsCell.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_GsPriceGoodsCell',
lock : true,
...
this.columns = [{text : '核算单元',width : 100,dataIndex : 'bean.cell',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysCell.List'}
	,{text : '基础价格信息',width : 100,dataIndex : 'bean.priceGoods',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'goods',mn : 'view.gs.GsPriceGoods.List'}
	...
	,{text : '零售标志',width : 100,dataIndex : 'bean.flagSal',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '批发标志',width : 100,dataIndex : 'bean.flagPf',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '调出标志',width : 100,dataIndex : 'bean.flagMvout',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '调入标志',width : 100,dataIndex : 'bean.flagMvin',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '采购标志',width : 100,dataIndex : 'bean.flagPur',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '产成品标志',width : 100,dataIndex : 'bean.flagFini',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '半成品标志',width : 100,dataIndex : 'bean.flagHalf',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '自用品标志',width : 100,dataIndex : 'bean.flagPriv',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '同步价格',width : 100,dataIndex : 'bean.syncFlag',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn')}
	,{text : '启用标志',width : 75,dataIndex : 'bean.enabled',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OEnabled')}
			...
			if (mainActs.length > 0)
				this.tbar=mainActs;
			this.store=Ext.create('mvc.store.gs.GsPriceGoodsCell'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.gs.GsPriceGoodsCell.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.gs.GsPriceGoodsCell', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gs_GsPriceGoodsCell_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/gs_GsPriceGoodsCell_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/gs_GsPriceGoodsCell_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/gs_GsPriceGoodsCell_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gs.GsPriceGoodsCell.WinSearch',{
