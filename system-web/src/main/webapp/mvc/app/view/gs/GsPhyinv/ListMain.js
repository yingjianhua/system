Ext.define('mvc.view.gs.GsPhyinv.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '预计完成日期',width : 100,dataIndex : 'bean.planFiniDate',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d')}
	,{text : '完成时间',width : 140,dataIndex : 'bean.finiTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	,{text : '不符品种数',width : 100,dataIndex : 'bean.unmatchNum',sortable : true}
	,{text : '金额差额',width : 100,dataIndex : 'bean.unmatchAmt',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
	,{text : '盘点员',width : 75,dataIndex : 'bean.countedBy',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysUser.List'}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		this.store=Ext.create('mvc.store.gs.GsPhyinv');
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/gs_GsPhyinv_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
