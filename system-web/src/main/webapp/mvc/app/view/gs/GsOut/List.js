Ext.define('mvc.view.gs.GsOut.List',{
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{xtype : 'textfield',name : 'bean.code',fieldLabel : '单据号'}
							...
							,{xtype : 'datefield',name : 'bean.outTime',fieldLabel : '实际出库时间',format : 'Y-m-d H:i:s'}
							,{
								xtype : 'beantrigger',
								name : 'bean.createdBy',
								fieldLabel : '建档员',
		...
		items : [{
				xtype : Ext.create('mvc.view.gs.GsOut.ListMain',{
							title : '出库单',
				...
				xtype : Ext.create('mvc.view.gs.GsOut.ListLineGsOutLineView',{
							title : '出库单明细',
...
onApproveRecord : function(form, data){
	var selection = this.mdMainTable.getView().getSelectionModel().getSelection()[0];
	var bean = Ext.create('mvc.model.gs.GsOut', data);
...
getGoodsKeys : function(pkey){
	...
	Ext.Ajax.request({
		async : false,
		url : base_path+'/gs_GsOutLineView_getGoodsKeys?pkey=' + pkey,
		method : 'GET',
...
onApprWin : function(selection){
	if (selection){
			mvc.model.gs.GsOut.load(selection.get('bean.pkey'), {
				scope : this,
				...
				success : function(response, operation) {
					...
					var win = Ext.create('mvc.view.gs.GsOut.ApprWin',{
						title : this.title+'>审核',
						insFlag : false,
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '出库单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/gs_GsOut_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.gs.GsOut',result);
								Ext.apply(selection.data, bean.data);
								selection.commit();
...
onPrint : function(){
		...
		window.open('/print/General/PrintReport.jsp?report=GsOut.grf&data=GsOut.jsp&pkey='+key,'_blank');		 
},
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gs.GsOut.WinSearch',{
