Ext.define('mvc.view.gs.GsSplit.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '合计费用',width : 100,dataIndex : 'bean.amtCost',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		mvc.Tools.doGoodsLine(this.columns, 3);		this.store=Ext.create('mvc.store.gs.GsSplit');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.gs.GsSplit', data);
...
onUpdWin : function(selection){
		if (selection){
			var win = Ext.create('mvc.view.gs.GsSplit.Win',{
				title : this.title+'>修改',
				insFlag : false
			});
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/gs_GsSplit_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
