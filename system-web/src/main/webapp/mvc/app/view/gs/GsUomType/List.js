Ext.define('mvc.view.gs.GsUomType.List',{
extend : 'Ext.panel.Panel',
oldId : 'GsUomType_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{xtype : 'textfield',name : 'bean.name',fieldLabel : '名称'}
							,{xtype : 'textfield',name : 'bean.shortkey',fieldLabel : '快捷键'}
		...
		items : [{
				xtype : Ext.create('mvc.view.gs.GsUomType.ListMain',{
							title : '计量单位类型',
        ...
        selectionchange: function(model, records) {
            if (records.length === 1){
				...
				this.mdLineTable.store.filter([{'id':'filter', 'property':'uomType','value':records[0].get('bean.pkey')}]);
				if (this.roles.indexOf('upd') != -1)
				...
				xtype : Ext.create('mvc.view.gs.GsUomType.ListLineGsUom',{
							title : '计量单位',
...
onIns : function(){
		var win = Ext.create('mvc.view.gs.GsUomType.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gs_GsUomType_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onEdit : function(){
	var selection = this.mdMainTable.getView().getSelectionModel().getSelection()[0];
	if (selection){
		var win = Ext.create('mvc.view.gs.GsUomType.WinEdit',{
	...
	if (selection) {
		...
		Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path + '/gs_GsUomType_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
						success : function(response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success) {
								me.getStore().reload();
	...
	if (selection) {
		...
		Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path + '/gs_GsUomType_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
						success : function(response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success) {
								me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gs.GsUomType.WinSearch',{
