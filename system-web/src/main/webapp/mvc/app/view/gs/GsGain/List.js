Ext.define('mvc.view.gs.GsGain.List',{
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.gs.GsGain.ListMain',{
							title : '盘盈单',
				...
				xtype : Ext.create('mvc.view.gs.GsGain.ListLineGsGainLine',{
							title : '盘盈单明细',
...
onIns : function(){
		var win = Ext.create('mvc.view.gs.GsGain.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gs_GsGain_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '盘盈单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/gs_GsGain_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.gs.GsGain',result);
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '盘盈单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/gs_GsGain_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.gs.GsGain',result);
...
onPrint : function(){
		...
		window.open('/print/General/PrintReport.jsp?report=GsGain.grf&data=GsGain.jsp&pkey='+key,'_blank');		 
},
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gs.GsGain.WinSearch',{
