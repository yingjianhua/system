Ext.define('mvc.view.gs.GsPhyinv.List',{
extend : 'Ext.panel.Panel',
oldId : 'GsPhyinv_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		text : '产生',
		iconCls : 'upd-icon',
		itemId : this.oldId+'produce',
		scope : this,
		handler : this.onProduce,
		disabled : this.lock
	});
		this.items =[{
		region : 'north',
		...
		items : [{
				xtype : Ext.create('mvc.view.gs.GsPhyinv.ListMain',{
							title : '盘点单',
				...
				xtype : Ext.create('mvc.view.gs.GsPhyinv.ListLineGsPhyinvGoodsLine',{
							title : '盘点商品明细',
...
onInvRecord : function(form, data){
...
onIns : function(){
		var win = Ext.create('mvc.view.gs.GsPhyinv.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gs_GsPhyinv_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '盘点单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/gs_GsPhyinv_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '盘点单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/gs_GsPhyinv_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.gs.GsPhyinv',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gs.GsPhyinv.WinSearch',{
