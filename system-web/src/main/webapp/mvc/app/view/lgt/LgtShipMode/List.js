Ext.define('mvc.view.lgt.LgtShipMode.List',{
	extend : 'Ext.grid.Panel',
	oldId : 'btn_LgtShipMode',
	lock : true,
	...
	initComponent : function(){
		...
		this.tbar=mainActs;
		this.store=Ext.create('mvc.store.lgt.LgtShipMode');
	...
	onIns : function(){
		var win = Ext.create('mvc.view.lgt.LgtShipMode.Win',{
			title : this.title+'>新增'
		});
	...
	onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.lgt.LgtShipMode', data);
	...
	onUpdWin : function(selection){
		if (selection){
			mvc.model.lgt.LgtShipMode.load(selection.get('bean.pkey'), {
				scope : this,
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/lgt_LgtShipMode_delMulti',
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/lgt_LgtShipMode_del?pkey='+me.getStore().getAt(rowIndex).get('bean.pkey'),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
