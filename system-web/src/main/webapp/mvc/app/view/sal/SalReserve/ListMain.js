Ext.define('mvc.view.sal.SalReserve.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '截止时间',width : 100,dataIndex : 'bean.expireDate',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d')}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		this.store=Ext.create('mvc.store.sal.SalReserve');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.sal.SalReserve', data);
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/sal_SalReserve_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
