Ext.define('mvc.view.sal.SalSaleDirect.List',{
extend : 'Ext.panel.Panel',
oldId : 'SalSaleDirect_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		scope : this,
		handler : this.onIns,
		menu : [{text:'从订单新增',iconCls : 'ins-icon', scope : this,handler: this.onInsOrderLoad}]
	});
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{xtype : 'textfield',name : 'bean.code',fieldLabel : '单据号'}
							...
							,{xtype : 'numberfield',name : 'bean.amtPay',fieldLabel : '现付金额',decimalPrecision : 2}
							,{xtype : 'numberfield',name : 'bean.amtRec',fieldLabel : '挂账金额',decimalPrecision : 2}
							,
		...
		items : [{
				xtype : Ext.create('mvc.view.sal.SalSaleDirect.ListMain',{
							title : '直销单',
        ...
        selectionchange: function(model, records) {
            if (records.length === 1){
				...
				xtype : Ext.create('mvc.view.sal.SalSaleDirect.ListLineSalSaleDirectLine',{
							title : '直销单明细',
...
onIns : function(){
		var win = Ext.create('mvc.view.sal.SalSaleDirect.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/sal_SalSaleDirect_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '直销单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/sal_SalSaleDirect_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.sal.SalSaleDirect',result);
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '直销单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/sal_SalSaleDirect_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.sal.SalSaleDirect',result);
...
onDoNote : function(){
	...
	if (selection){
		var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
			...
			insFlag : false,
			tableCode : 'irille.pss.sal.SalSaleDirect'
		});
		win.show();
...
onDoTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '直销单['+selection.get('bean.code') + '] - 记账确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/sal_SalSaleDirect_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.sal.SalSaleDirect',result);
...
onUnTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '直销单['+selection.get('bean.code') + '] - 记账取消确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/sal_SalSaleDirect_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.sal.SalSaleDirect',result);
...
onPrint : function(){
		...
		window.open('/print/General/PrintReport.jsp?report=SalSaleDirect.grf&data=SalSaleDirect.jsp&pkey='+key,'_blank');		 
},
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.sal.SalSaleDirect.WinSearch',{
...
onInsOrder : function(data, params){
			var win = Ext.create('mvc.view.sal.SalSaleDirect.Win',{
				title : '直销单>从销售订单['+data.split('##')[1]+']新增'
			});
			win.on('create',this.onSaveRecord,this);
			win.show();
