Ext.define('mvc.view.sal.SalPresent.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '机构',width : 100,dataIndex : 'bean.org',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysOrg.List'}
		...
		this.store=Ext.create('mvc.store.sal.SalPresent');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.sal.SalPresent', data);
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/sal_SalPresent_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
