Ext.define('mvc.view.sal.SalSale.Win',{
...
initComponent : function(){
		...
		anchor : '100%',
		plain : true,
		xtype : Ext.create('mvc.view.sal.SalSale.Form',{	insFlag : this.insFlag})
	},{
		xtype : Ext.create('mvc.view.sal.SalSale.ListForm',{height : 250,border : false })
	}];
...
setActiveOrder : function(orderPkey){
	...
	Ext.Ajax.request({
		...
		success : function(response) {
			...
			var row  = Ext.create('mvc.model.sal.SalSale');
...
initOrderCmp : function(){
	this.form.getForm().getFields().each(function(fd){
		if (fd.name!='bean.amtOrd' && fd.name!='bean.amtPay' && fd.name!='bean.amtRec' && fd.name != 'bean.warehouse')
			fd.setReadOnly(true);
	});
