Ext.define('mvc.view.sal.SalSaleDirect.ListForm',{
...
,{text : '数量',width : 100,dataIndex : 'bean.qty',sortable : true,align : 'right',renderer : mvc.Tools.numberRenderer(4),editor : {xtype : 'numberfield',decimalPrecision : 4}
	...
	,{text : '计量单位',width : 80,dataIndex : 'bean.uom',sortable : true,renderer : mvc.Tools.beanRenderer(),
		editor : {
			xtype : 'beantriggercell',bean : 'GsUom',
			beanType : 'gs',beanName : 'bean.uom',
			grid : this,emptyText : form_empty_text,
			onTrigger : function(data, params) {
				this.setValue(data);
				var row = this.grid.getView().getSelectionModel().getSelection()[0];
				row.set(this.beanName,this.getValue());
				var cust = this.grid.up('window').down('form').down('[name=bean.cust]').getValue();
				var goodsCode = row.get('bean.goods').split(bean_split)[1];
	...
	,{text : '金额',width : 120,dataIndex : 'bean.amt',decimalPrecision : 2,sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'
		}
	];
		mvc.Tools.doGoodsLine(this.columns, 1);		this.store=Ext.create('mvc.store.sal.SalSaleDirectLine');
...
onIns : function(){
		var model = Ext.create('mvc.store.sal.SalSaleDirectLine');
...
onLoadInfoPrice : function(newv, row){
	...
	var cust = this.up('window').down('form').down('[name=bean.cust]').getValue();
	if (!cust){
...
doLoadPrice : function(row, cust, goodsCode, uom){
	var urlGoods = base_path+ '/sal_SalPriceProt_loadInfoPrice?cust=' + cust + '&goods=' + goodsCode;
	if (uom)
		urlGoods += '&uom=' + uom;
	Ext.Ajax.request({
