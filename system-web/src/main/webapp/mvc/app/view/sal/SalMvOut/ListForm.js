Ext.define('mvc.view.sal.SalMvOut.ListForm',{
insFd : false,
chkFlag : false,
...
initComponent : function(){
	if(!this.insFd && !this.chkFlag){		
		...
		//,{text : '分摊费用',width : 100,dataIndex : 'bean.amtCost',sortable : true,renderer : mvc.Tools.numberRenderer4(),align : 'right',editor : {xtype : 'numberfield',decimalPrecision : 2}}
		];
	}else{
		this.columns =[		
		{text : '货物', width : 100, dataIndex : 'bean.goods', sortable : true, renderer : mvc.Tools.beanRenderer()}
		...
		//,{text : '分摊费用',width : 100,dataIndex : 'bean.amtCost',sortable : true,renderer : mvc.Tools.numberRenderer(4),align : 'right',editor : {xtype : 'numberfield',decimalPrecision : 2}}
		];
	}
	
	mvc.Tools.doGoodsLine(this.columns, 1);				
	this.store=Ext.create('mvc.store.sal.SalMvOutLine');
...
onIns : function(){
		var model = Ext.create('mvc.store.sal.SalMvOutLine');
