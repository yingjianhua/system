...
this.columns = [{text : '核算单元',width : 100,dataIndex : 'bean.cell',sortable : true,renderer : mvc.Tools.beanRenderer()}
	...
	,{text : '价格级别',width : 100,dataIndex : 'bean.priceLevel',sortable : true}
	,{
		xtype : 'actioncolumn',
		width : 80,
		sortable : false,
...
onUpdWin : function(selection){
		if (selection){
			mvc.model.sal.SalPriceProt.load(selection.get('bean.pkey'), {
				scope : this,
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/sal_SalPriceProbdelMulti',
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/sal_SalPriceProt_del?pkey='+me.getStore().getAt(rowIndex).get('bean.pkey'),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
