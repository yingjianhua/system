Ext.define('mvc.view.sys.SysCtypeCode.ListEdit',{
...
var mainActs =[{
		...
		handler : this.onReset
			...
			if (this.isEdit)
				this.tbar=mainActs;
this.columns = [{text : '参数代码',width : 120,dataIndex : 'bean.codeValue',sortable : true,editor : {}
		}
	,{text : '参数名称',width : 150,dataIndex : 'bean.codeName',sortable : true,editor : {}
		}
	,{text : '描述',width : 200,dataIndex : 'bean.codeDes',sortable : true,editor : {}
		}
	];
		this.store=Ext.create('mvc.store.sys.SysCtypeCode');
...
onIns : function(){
		var model = Ext.create('mvc.store.sys.SysCtypeCode');
...
onSave : function(){
		...
		if (change){
			Ext.Ajax.request({
				url : base_path+'/sys_SysCtypeCode_sync?mainPkey='+me.mainPkey,
				params : syncValues,
...
onReset : function(){
		this.store.filter([{'property':'ctypeType','value':this.mainPkey}]);
},
