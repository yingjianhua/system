Ext.define('mvc.view.gl.GlTransform.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '类型',width : 100,dataIndex : 'bean.type',sortable : true,renderer : mvc.Tools.optRenderer('gl','Gl','OTransformType')}
	];
		this.store=Ext.create('mvc.store.gl.GlTransform');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.gl.GlTransform', data);
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/gl_GlTransform_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
