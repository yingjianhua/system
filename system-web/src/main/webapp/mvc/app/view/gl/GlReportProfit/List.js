Ext.define('mvc.view.gl.GlReportProfit.List', {
	extend : 'Ext.panel.Panel',
	oldId : 'GlReportProfit_list_',
	loadMask : true,
	multiSelect : true,
	roles : '',
	layout : 'border',
		...
		this.items = [{
					...
					items : [{
						...
						items : [{
							...
							region : 'center',
							items : Ext.create(
									'mvc.view.gl.GlReportProfit.ListMain', {
										width : 300,
										height : "100%",
										...
										listeners : {
											...
											selectionchange : function(model,records) {
												if (records.length === 1) {
													this.mdLineTable.store.proxy.url = base_path+ '/gl_GlReportProfitLine_list?bean.profitReport='+ records[0].get('bean.pkey');
						...
						items : Ext
								.create(
										'mvc.view.gl.GlReportProfit.ListLineGlReportProfitLine',
										{
	...
	onIns : function() {
		var win = Ext.create('mvc.view.gl.GlReportProfit.Win', {
					title : this.title + '>新增'
				});
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/gl_GlReportProfit_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
	...
	onPrint : function(){
		...
		window.open('/print/General/PrintReport.jsp?report=GlReportProfit.grf&data=GlReportProfit.jsp&pkey='+key,'_blank');		 
	}
});