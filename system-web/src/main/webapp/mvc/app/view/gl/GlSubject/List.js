Ext.define('mvc.view.gl.GlSubject.List',{
...
this.columns = [{text : '科目号',width : 100,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '上级科目',width : 100,dataIndex : 'bean.subjectUp',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'gl',mn : 'view.gl.GlSubject.List'}
	,{text : '名称',width : 170,dataIndex : 'bean.name',sortable : true}
	,{text : '分类',width : 100,dataIndex : 'bean.subjectKind',sortable : true,renderer : mvc.Tools.optRenderer('gl','Gl','OSubjectKind'),expandCol : true,hidden : true}
	...
	,{text : '应用范围',width : 150,dataIndex : 'bean.useScope',sortable : true,renderer : mvc.Tools.optRenderer('gl','Gl','OUseScope')}
	,{text : '账户类型',width : 100,dataIndex : 'bean.accType',sortable : true,renderer : mvc.Tools.optRenderer('gl','Gl','OAccType')}
	,{text : '可否自动建账户',width : 100,dataIndex : 'bean.autoCrt',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OYn'),expandCol : true,hidden : true}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true,expandCol : true,hidden : true}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.gl.GlSubject'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.gl.GlSubject.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.gl.GlSubject', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gl_GlSubject_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/gl_GlSubject_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/gl_GlSubject_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/gl_GlSubject_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gl.GlSubject.WinSearch',{
