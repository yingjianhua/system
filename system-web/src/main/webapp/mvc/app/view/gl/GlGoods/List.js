Ext.define('mvc.view.gl.GlGoods.List',{
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{
							...
							,{xtype : 'numberfield',name : 'bean.qty',fieldLabel : '数量',decimalPrecision : 4}
							,{xtype : 'numberfield',name : 'bean.price',fieldLabel : '单价',decimalPrecision : 4}
		...
		items : [{
				xtype : Ext.create('mvc.view.gl.GlGoods.ListMain',{
							title : '存货账',
							...
							listeners : 							 {
				                ...
				                selectionchange: function(model, records) {
				                    if (records.length === 1){
        								...
        								this.mdLineTable.store.filter([{'id':'filter', 'property':'goods','value':records[0].get('bean.pkey')}]);
    									if (this.roles.indexOf('upd') != -1)
				...
				xtype : Ext.create('mvc.view.gl.GlGoods.ListLineGlGoodsLine',{
							title : '存货账明细',
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/gl_GlGoods_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.gl.GlGoods.WinSearch',{
