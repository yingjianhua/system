Ext.define('mvc.view.pya.PyaPayOtherBill.List',{
extend : 'mvc.tools.RowexpanderGrid',
oldId : 'btn_PyaPayOtherBill',
lock : true,
...
this.columns = [{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '机构',width : 100,dataIndex : 'bean.org',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysOrg.List'}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.pya.PyaPayOtherBill'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.pya.PyaPayOtherBill.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.pya.PyaPayOtherBill', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/pya_PyaPayOtherBill_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pya_PyaPayOtherBill_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '其它应付单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/pya_PyaPayOtherBill_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.pya.PyaPayOtherBill',result);
								Ext.apply(selection.data, bean.data);
								selection.commit();
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '其它应付单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/pya_PyaPayOtherBill_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.pya.PyaPayOtherBill',result);
...
onDoNote : function(){
	...
	if (selection){
		var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
			...
			insFlag : false,
			tableCode : 'irille.gl.pya.PyaPayOtherBill'
		});
		win.show();
		win.setActiveRecord(selection);
	}
...
onDoTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '其它应付单['+selection.get('bean.code') + '] - 记账确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pya_PyaPayOtherBill_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.pya.PyaPayOtherBill',result);
...
onUnTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '其它应付单['+selection.get('bean.code') + '] - 记账取消确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/pya_PyaPayOtherBill_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.pya.PyaPayOtherBill',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.pya.PyaPayOtherBill.WinSearch',{
