Ext.define('mvc.view.rp.RpNotePayBill.List',{
extend : 'mvc.tools.RowexpanderGrid',
oldId : 'btn_RpNotePayBill',
lock : true,
...
this.columns = [{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '机构',width : 100,dataIndex : 'bean.org',sortable : true,renderer : mvc.Tools.beanRendererHref(),expandCol : true,hidden : true,md : 'sys',mn : 'view.sys.SysOrg.List'}
	...
	,{text : '出纳日记账',width : 100,dataIndex : 'bean.journal',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'rp',mn : 'view.rp.RpJournal.List'}
	...
	,{text : '付款时间',width : 140,dataIndex : 'bean.tranTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	,{text : '票据日期',width : 100,dataIndex : 'bean.date',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d')}
	,{text : '实际付款日期',width : 100,dataIndex : 'bean.payDate',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d')}
	,{text : '收款方名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '收款方账号',width : 100,dataIndex : 'bean.account',sortable : true}
	,{text : '收款方银行',width : 100,dataIndex : 'bean.bank',sortable : true}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.rp.RpNotePayBill'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.rp.RpNotePayBill.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.rp.RpNotePayBill', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/rp_RpNotePayBill_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/rp_RpNotePayBill_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '付款单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpNotePayBill_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '付款单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpNotePayBill_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.rp.RpNotePayBill',result);
...
onDoNote : function(){
	...
	if (selection){
		var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
			...
			insFlag : false,
			tableCode : 'irille.gl.rp.RpNotePayBill'
		});
		win.show();
		win.setActiveRecord(selection);
	}
...
onDoTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '付款单['+selection.get('bean.code') + '] - 记账确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/rp_RpNotePayBill_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.rp.RpNotePayBill',result);
...
onUnTally : function(){
	...
	var me = this;
	if (selection){
		Ext.MessageBox.confirm(msg_confirm_title, '付款单['+selection.get('bean.code') + '] - 记账取消确认吗？',
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/rp_RpNotePayBill_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							var bean  = Ext.create('mvc.model.rp.RpNotePayBill',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.rp.RpNotePayBill.WinSearch',{
