Ext.define('mvc.view.rp.RpFundMv.List',{
...
mainActs.push({
		text : '接收',
		iconCls : 'doAppr-icon',
		itemId : this.oldId+'doRec',
		scope : this,
		handler : this.onDoRec,
		disabled : this.lock
	});
...
mainActs.push({
		text : '接收取消',
		iconCls : 'doAppr-icon',
		itemId : this.oldId+'unRec',
		scope : this,
		handler : this.onUnRec,
		disabled : this.lock
	});
...
this.columns = [{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '来源账户',width : 100,dataIndex : 'bean.sourceJournal',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'rp',mn : 'view.rp.RpJournal.List'}
	,{text : '交出人',width : 75,dataIndex : 'bean.sourceBy',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysUser.List'}
	,{text : '交出时间',width : 140,dataIndex : 'bean.sourceTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	,{text : '接收账户',width : 100,dataIndex : 'bean.descJournal',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'rp',mn : 'view.rp.RpJournal.List'}
	,{text : '接收人',width : 75,dataIndex : 'bean.descBy',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysUser.List'}
	,{text : '接收时间',width : 140,dataIndex : 'bean.descTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.rp.RpFundMv'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.rp.RpFundMv.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.rp.RpFundMv', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/rp_RpFundMv_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onDoAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') + '] - 审核确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_approve?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
								Ext.apply(selection.data, bean.data);
								selection.commit();
...
onUnAppr : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') + '] - 弃审确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_unapprove?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
...
onDoRec : function(){
		...
		var me = this;
		if(selection) {
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') +'] -确认接收吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_doRec?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
							success : function (response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success){
									var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
...
onUnRec : function(){
		...
		var me = this;
		if(selection) {
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') +'] -确认取消接收吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_unRec?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
							success : function (response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success){
									var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
...
onDoNote : function(){
		...
		if (selection){
			var win = Ext.create('mvc.view.gl.GlNote.WinNote',{
				...
				insFlag : false,
				tableCode : 'irille.gl.rp.RpFundMv'
			});
			win.show();
			win.setActiveRecord(selection);
		}
...
onDoTally : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') + '] - 记账确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_tally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
...
onUnTally : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '资金调拨单['+selection.get('bean.code') + '] - 记账取消确认吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpFundMv_untally?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
								var bean  = Ext.create('mvc.model.rp.RpFundMv',result);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.rp.RpFundMv.WinSearch',{
