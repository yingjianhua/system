Ext.define('mvc.view.rp.RpHandover.ListMain',{
...
this.columns =[{text : '单据号',width : 135,dataIndex : 'bean.code',sortable : true}
	...
	,{text : '交出人',width : 75,dataIndex : 'bean.source',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysUser.List'}
	,{text : '交出时间',width : 140,dataIndex : 'bean.giveUpTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	,{text : '收入的工作箱',width : 100,dataIndex : 'bean.workBox',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'rp',mn : 'view.rp.RpWorkBox.List'}
	,{text : '收入的工作箱名称',width : 100,dataIndex : 'bean.workBoxName',sortable : true}
	,{text : '接交人',width : 75,dataIndex : 'bean.descBy',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'sys',mn : 'view.sys.SysUser.List'}
	,{text : '接收时间',width : 140,dataIndex : 'bean.takeOverTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true,expandCol : true,hidden : true}
		...
		this.store=Ext.create('mvc.store.rp.RpHandover');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.rp.RpHandover', data);
...
onUpdWin : function(selection){
		if (selection){
			var win = Ext.create('mvc.view.rp.RpHandover.Win',{
				title : this.title+'>修改',
				insFlag : false
			});
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/rp_RpHandover_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
