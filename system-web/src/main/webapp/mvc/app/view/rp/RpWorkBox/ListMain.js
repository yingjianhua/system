Ext.define('mvc.view.rp.RpWorkBox.ListMain',{
...
this.columns =[{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	...
	,{text : '备注',width : 100,dataIndex : 'bean.rem',sortable : true}
		...
		this.store=Ext.create('mvc.store.rp.RpWorkBox');
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.rp.RpWorkBox', data);
...
onDelRow : function(grid, rowIndex){
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				...
				Ext.Ajax.request({
					url : '/rp_RpWorkBox_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
