Ext.define('mvc.view.rp.RpStimatePay.List',{
extend : 'Ext.panel.Panel',
oldId : 'RpStimatePay_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				xtype : Ext.create('mvc.view.rp.RpStimatePay.ListMain',{
							title : '待付款登记',
				...
				xtype : Ext.create('mvc.view.rp.RpStimatePay.ListLineGlNoteViewRp',{
							title : '记账条视图',
...
onDoProcRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.rp.RpStimatePay', data);
...
onDoProc : function(){
		var selection = this.mdMainTable.getView().getSelectionModel().getSelection()[0];
		if (selection){
			var win = Ext.create('mvc.view.rp.RpStimatePay.WinProc',{
				title : this.title+'>付款',
			});
...
onUnProc : function(){
		...
		var me = this;
		if (selection){
			Ext.MessageBox.confirm(msg_confirm_title, '付款待处理单据['+selection.get('bean.origFormNum') + '] - 确认取消付款吗？',
				function(btn) {
					if (btn != 'yes')
						return;
					Ext.Ajax.request({
						url : base_path+'/rp_RpStimatePay_unProc?pkey='+selection.get('bean.pkey'),
						success : function (response, options) {
							...
							if (result.success){
								...
								Ext.example.msg(msg_title, '取消付款--成功');
							}else{
								Ext.MessageBox.show({
									title : msg_title, 
									msg : result.msg,
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.rp.RpStimatePay.WinSearch',{
